...
import { drawSplitImage, exportAndDownload } from "../../utils/canvas";
...
function ImagePlaceholderIcon() {
  return (
    <svg width="48" height="48" viewBox="0 0 48 48" fill="none" aria-hidden="true">
      <rect x="4" y="10" width="18" height="28" rx="3" stroke="currentColor" strokeWidth="2" strokeDasharray="4 3" />
      <rect x="26" y="10" width="18" height="28" rx="3" stroke="currentColor" strokeWidth="2" strokeDasharray="4 3" />
      <path d="M22 10 L26 38" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeDasharray="3 3" />
      <circle cx="11" cy="19" r="3" stroke="currentColor" strokeWidth="1.5" />
      <path d="M4 32 l5-6 4 4 3-3 6 8" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
      <circle cx="37" cy="19" r="3" stroke="currentColor" strokeWidth="1.5" />
      <path d="M26 32 l5-6 4 4 3-3 6 8" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
  ...
  return (
    ...
    >
      ...
      <circle
        ...
        strokeLinecap="round" strokeDasharray="42" strokeDashoffset="14"
        opacity="0.85"
      />
    </svg>
  );
...
function EmptyState({ hasLight, hasDark }: { hasLight: boolean; hasDark: boolean }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-4 p-10 select-none">
      <div
        ...
        style={{ color: "var(--color-text-tertiary)", background: "var(--color-surface-secondary)" }}
      >
      ...
      <div className="flex items-center gap-2 mt-1">
        <StepDot label="亮色" done={hasLight} />
        <div className="w-8 h-px" style={{ background: "var(--color-border)" }} />
        <StepDot label="暗色" done={hasDark} />
        <div className="w-8 h-px" style={{ background: "var(--color-border)" }} />
...
function StepDot({ label, done, dimmed }: { label: string; done: boolean; dimmed?: boolean }) {
  return (
    <div className="flex flex-col items-center gap-1">
      ...
      <span
        ...
        style={{ color: dimmed ? "var(--color-text-tertiary)" : "var(--color-text-secondary)" }}
      >
        {label}
      </span>
    </div>
...
function RenderingOverlay() {
  return (
    <div
      className="absolute inset-0 flex items-center justify-center rounded-[var(--radius-xl)] z-10"
...
function DimensionBadge({ width, height, scale }: { width: number; height: number; scale: number }) {
  ...
  return (
    ...
    >
      ...
      />
      <span>{exportW} × {exportH}</span>
      {scale > 1 && <span style={{ opacity: 0.6 }}>@{scale}×</span>}
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col gap-3 h-full">
      ...
      <div
        ...
        style={{
          background: hasOutput ? "transparent" : "var(--color-surface-secondary)",
      ...
      >
        ...
        {!hasBoth && !isRendering && (
          <EmptyState hasLight={hasLight} hasDark={hasDark} />
        )}
        ...
        {error && (
          <div className="absolute inset-0 flex items-center justify-center z-20 p-6">
            ...
            >
              ...
              <span className="text-xs text-[var(--color-text-secondary)] leading-relaxed">{error}</span>
            </div>
          </div>
        )}

        {hasOutput && previewW > 0 && previewH > 0 && (
          <DimensionBadge width={previewW} height={previewH} scale={exportConfig.scale} />
        )}
      </div>
